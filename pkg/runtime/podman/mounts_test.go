...
func TestValidateMount(t *testing.T) {
	...
	tests := []struct {
		...
		// $SOURCES-based targets
		{
			name:        "$SOURCES root is valid",
			target:      "$SOURCES",
			expectError: false,
		},
		{
			name:        "$SOURCES subpath is valid",
			target:      "$SOURCES/pkg",
			expectError: false,
		},
		{
			name:        "$SOURCES parent traversal staying within /workspace is valid",
			target:      "$SOURCES/../other",
			expectError: false,
		},
		{
			name:        "$SOURCES escaping above /workspace is invalid",
			target:      "$SOURCES/../../etc",
			expectError: true,
		},
		// $HOME-based targets
		{
			name:        "$HOME root is valid",
			target:      "$HOME",
			expectError: false,
		},
		{
			name:        "$HOME subpath is valid",
			target:      "$HOME/.ssh",
			expectError: false,
		},
		{
			name:        "$HOME escaping above container home is invalid",
			target:      "$HOME/../../etc",
			expectError: true,
		},
		// Absolute targets
		{
			name:        "absolute target is accepted as-is",
			target:      "/workspace/sources",
			expectError: false,
		},
		{
			name:        "absolute target outside workspace is accepted",
			target:      "/tmp/data",
			expectError: false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			m := workspace.Mount{Host: "/host/path", Target: tc.target}
			err := validateMount(m)
			if tc.expectError && err == nil {
				t.Errorf("validateMount() expected error for target %q, got nil", tc.target)
			}
			if !tc.expectError && err != nil {
				t.Errorf("validateMount() unexpected error for target %q: %v", tc.target, err)
			}
		})
	}
}
